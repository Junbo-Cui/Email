...
public class FolderListFragment extends ListFragment implements
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Drawable drawable = getActivity().getResources().getDrawable(R.drawable.linearlayout_selector);
    ...
    protected void viewFolderOrChangeAccount(int position) {
        ...
        if (folder != null) {
            ...
            if(folder.name.equals(getResources().getString(R.string.mailbox_name_display_search))){
             controller.SearchViewClick();
            }else{
    ...
    public void onFolderSelected(Folder folder, String analyticsLabel) {
        ...
        //if (!folder.folderUri.equals(mSelectedFolderUri)) {
            mNextFolder = folder;
            ConversationListFragment.mSelectFolder = folder;
            ...
            ActionBarController actionbar = new ActionBarController(mActivity.getActivityContext());
           // actionbar.setNewFolderAndAccount(folder.name);
            MailActivity mainActivity = new MailActivity();
            mainActivity.SetActionBarTitle(folder.name);

            Log.i("lhz1220","folder.name:"+folder.name);
            OnePaneController.ACTION_BAR_TITLE_FOLDER = folder.name;
            mListView.requestFocus();
            mListView.setItemChecked(0,true);
            mListView.setSelection(0);
       ...
       // }
        /*
            ...
            mAccountController.closeDrawer(false ,
                    null ,
                    folder );
            MailActivity mainActivity = new MailActivity();
            mainActivity.SetActionBarTitle(folder.name);
            Log.i("lhz1220","folder.name2:"+folder.name);
            mListView.requestFocus();
            mListView.setItemChecked(0,true);
            mListView.setSelection(0);
        }
    ...
    private String onSetMainTitle(String name){
        switch (name) {
         case "Inbox":
           return getResources().getString(R.string.mailbox_name_display_inbox);
         case "Outbox":
           return getResources().getString(R.string.mailbox_name_display_outbox);
         case "Drafts":
           return getResources().getString(R.string.mailbox_name_display_drafts);
         case "Trash":
           return getResources().getString(R.string.mailbox_name_display_trash);
         case "Sent":
           return getResources().getString(R.string.mailbox_name_display_sent);
         case "Junk":
           return getResources().getString(R.string.mailbox_name_display_junk);
         case "Starred":
           return getResources().getString(R.string.mailbox_name_display_starred);
         case "Unread":
           return getResources().getString(R.string.mailbox_name_display_unread);                                               
        default:
       }
       return getResources().getString(R.string.mailbox_name_display_inbox);
    }
    ...
    private class FolderAdapter extends BaseAdapter implements FolderListFragmentCursorAdapter {
        ...
        private List<DrawerItem> recalculateDividedListFolders(List<DrawerItem> itemList) {
            ...
            if(mAllFolderListCursor != null){
               Folder folderSearch = mAllFolderListCursor.getModel();
               folderSearch.name = getResources().getString(R.string.mailbox_name_display_search);
               inboxFolders.add(DrawerItem.ofFolder(
                                mActivity, folderSearch, DrawerItem.FOLDER_INBOX));
            }    
            do {
                final Folder f = mCursor.getModel();
                if (!isFolderTypeExcluded(f)) {
                    if (f.isInbox()) {
                        ...
                        //f.name = getResources().getString(R.string.mailbox_name_display_inbox);
                        //inboxFolders.add(getResources().getString(R.string.mailbox_name_display_search));
                        //f.name = getResources().getString(R.string.mailbox_name_display_search);
                    } else {
                        if(f.name.equals(getResources().getString(R.string.mailbox_name_display_search))){
                           f.name = getResources().getString(R.string.mailbox_name_display_starred);
                        }
