...
public class ConversationViewHeader extends LinearLayout implements OnClickListener,
    ...
    protected void onFinishInflate() {
        ...
        cvh = (ConversationViewHeader)findViewById(R.id.conversation_header);
        mSubjectAndFolderView.setOnLongClickListener(this);
        cvh.setFocusableInTouchMode(true);
        cvh.requestFocus();
        ...
        cvh.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View view, boolean b) {
              if(b){
                 mSubjectAndFolderView.setTextColor(Color.parseColor("#FFFFFF"));
              }else{
                 mSubjectAndFolderView.setTextColor(Color.parseColor("#000000"));
              }
            }
        });
    }
    ...
    public static void setNewStarred(boolean isStarred) {
        mStarView.setStarred(isStarred);
        boolean showStar = mConversation != null && !mConversation.isInTrash();
        mStarView.setStarred(isStarred);
        mStarView.setVisibility(showStar ? View.VISIBLE : View.INVISIBLE);
    ...
    public void onConversationUpdated(Conversation conv) {
        ...
        mConversation = conv;
        Log.i("lhz0321","onConversationUpdated");
        ...
        //mainActivity.SetActionBarTitle(conv.subject);
        if(OnePaneController.ACTION_BAR_TITLE_FOLDER!=null){
            mainActivity.SetActionBarTitle(OnePaneController.ACTION_BAR_TITLE_FOLDER);
        }else{
            mainActivity.SetActionBarTitle(getResources().getString(R.string.mailbox_name_display_inbox));
        }
    }
    ...
    public static void addStarInInfo(){
        if(mConversation != null){
            mConversation.starred = !mConversation.starred;
            setNewStarred(mConversation.starred);
            mConversationUpdater.updateConversation(Conversation.listOf(mConversation),
                    UIProvider.ConversationColumns.STARRED, mConversation.starred);
        }else{
          Log.i("lhz1129","33333333333");
        }
    }
