...
public class AccountSetupFinal extends AccountSetupActivity
                ...
                if (shouldDivertToManual()) {
                    ...
                    if (mIsPreConfiguredProvider) {
                        ...
                        if (!TextUtils.isEmpty(amProtocol)) {
                            ...
                            final Account account = mSetupData.getAccount();
                            mSetupData.setIncomingProtocol(this, "imap");
                            setDefaultsForProtocol(account);
                            mState = STATE_CREDENTIALS;
                        }
                    }
    ...
    private boolean onBasicsComplete() {
        ...
        //if (mProvider != null && /* M: don't auto setup in manual*/ !shouldDivertToManual()) {
          if(AccountSetupBasicsFragment.EMAIL_ACCOUNT_VALUE.contains("gmail.com")){
            mIsPreConfiguredProvider = true;
    ...
    private AccountSetupBasicsFragment getAccountSetupFragment() {
        final AccountSetupBasicsFragment f = (AccountSetupBasicsFragment) getContentFragment();
        return f;
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if(AccountSetupTypeFragment.OnkeyDownFlag == 0){
           final AccountSetupBasicsFragment f = (AccountSetupBasicsFragment) getContentFragment();
           f.onKeyDown(keyCode,event);
        }else if(AccountSetupTypeFragment.OnkeyDownFlag == 1){
           final AccountSetupTypeFragment af =  (AccountSetupTypeFragment) getContentFragment();
           af.onKeyDown(keyCode,event);
        }else if(AccountSetupTypeFragment.OnkeyDownFlag == 2){
            final AccountSetupCredentialsFragment acf = (AccountSetupCredentialsFragment)
                getContentFragment();
            acf.onKeyDown(keyCode,event);
        }else if(AccountSetupTypeFragment.OnkeyDownFlag == 3){
            AccountServerBaseFragment asf = (AccountServerBaseFragment) getContentFragment();
            asf.onKeyDown(keyCode,event);
        }else if(AccountSetupTypeFragment.OnkeyDownFlag == 4){
            final AccountSetupOptionsFragment asof = (AccountSetupOptionsFragment)
                getContentFragment();
            asof.onKeyDown(keyCode,event);
        }else if(AccountSetupTypeFragment.OnkeyDownFlag == 5){
    ...
    private void collectCredentials() {
        ...
        if (info != null && info.usesSmtp) {
            ...
            AccountSetupCredentialsFragment.populateHostAuthWithResults(this, sendAuth,
                    mSetupData.getCredentialResults());
            Log.i("lhz0609","setOutgoingCredLoaded");
